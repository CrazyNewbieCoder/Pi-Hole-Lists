            ...
            for line in f:
                ...
                if line.startswith("0.0.0.0 "):
                    ...
                    if domain and " " not in domain:
                        domains.add("0.0.0.0 " + domain.rstrip("\n"))
                else:
                    # Перевірка на валідність домену
                    if re.match(r"^[a-zA-Z0-9][-a-zA-Z0-9.]*\.[a-zA-Z]{2,}$", line):
                        domains.add("0.0.0.0 " + line.rstrip("\n"))
    return domains


def write_blocklist(domains, out_path):
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("#------------------------[UPDATE]------------------------\n")
        f.write("# Title: Blocklist\n")
        f.write(
            f'# Updated:: {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")}\n'
        )
        f.write(f"# Total number of blocklist urls: {str(len(domains))}\n")
        f.write("# -------------------------[URL]-------------------------\n")
        for d in domains:
            f.write(f"{d}\n")
...
if __name__ == "__main__":
    ...
    blocklist = load_domains(Path.joinpath(base_dir, "blocklist-src"))
    # Path.joinpath(base_dir, "release").mkdir(exist_ok=True)
    blocklist_path = str(Path.joinpath(base_dir, "blocklist.txt"))
    write_blocklist(blocklist, blocklist_path)
